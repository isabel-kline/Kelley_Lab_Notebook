...
setwd("~/Documents/Github/Kelley_Lab_Notebook")
library(ggplot2)
library(tidyverse)
Input_Yield <- read_csv("Input_Yield.csv")

Input_Yield$Batch<-as.factor(Input_Yield$Batch_number)
Input_Yield %>%
  ggplot(aes(x= `Input Sample weight  (mg)`, y= `Yield (ng)`, color = `Batch`))+
  geom_point()+
  theme_minimal()
...
Input_Yield %>%
  ggplot(aes(x= `Input Sample weight  (mg)`, y= `260/280`, color = `Batch`))+
  geom_point()+
  theme_minimal()

Input_Yield %>%
  ggplot(aes(y= `Input Sample weight  (mg)`, x= `260/230`, color = `Batch`))+
  geom_point()+
  theme_minimal()
```
